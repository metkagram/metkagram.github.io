...
export function migrateAnnotations() {
  ...
  const report = {
    ...
    warnings: [],
    errors: [],
    checks: { missing_translations: 0, incomplete_examples: 0, recovered_text: 0 }
  };
  ...
  for (const target of Object.values(targetMeta)) {
    for (const collection of collectionKeys) {
      ...
      let sourceCount = 0;
      for (const document of read(file)) {
        for (const annotation of document.annotations || []) {
          const record = legacyAnnotationToCanonical(annotation, {
            language: target.dataKey,
            locale: target.dataKey,
            dataset: `${target.dataKey}/${collection}`,
            document_id: document.id
          });
          const errors = validateAnnotation(record);
          if (errors.length) report.errors.push({ id: record.id, errors });
          if (!annotation.original_text) {
            report.warnings.push({ id: record.id, warning: "original_text missing; recovered from legacy text_span" });
            report.checks.recovered_text += 1;
          }
      ...
      report.sources.push({ dataset: `${target.dataKey}/${collection}`, records: sourceCount });
    }
  }
  ...
  for (const record of patternCards) {
    const errors = validateAnnotation(record);
    if (errors.length) report.errors.push({ id: record.id, errors });
    if (!record.examples.length || record.examples.some((example) => !example.text || !example.translation)) {
      report.checks.incomplete_examples += 1;
    }
  }

  const ids = new Set();
  for (const record of [...records, ...patternCards]) {
    if (ids.has(record.id)) report.errors.push({ id: record.id, errors: ["duplicate record id"] });
    ids.add(record.id);
  }
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  if (result.report.errors.length) {
    console.error(JSON.stringify(result.report.errors, null, 2));
    process.exitCode = 1;
  } else {
    console.log(`Migrated ${result.records.length} sentences and ${result.patternCards.length} pattern cards (${result.report.warnings.length} recovery warnings).`);
  }
}
