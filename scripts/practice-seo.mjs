...
function patchPracticeIndex(locale, content) {
  ...
  const section = `<details id="learning-paths" class="practice-secondary-path ruled" data-practice-secondary="paths"><summary><span>${ru ? "Продолжить по маршруту" : "Continue with a route"}</span><strong>${ru ? "Учебные маршруты" : "Learning paths"}</strong><small>${content.studySets.learningPaths.length} ${ru ? "маршрутов по тематическим сетам" : "routes through topic sets"}</small></summary><div class="practice-secondary-body research-questions section-pad"><div><h2>${ru ? "От задачи к набору, от набора к модели" : "From learning goal to set to pattern"}</h2><p>${ru ? `${content.studySets.sets.length} учебных наборов сгруппированы в ${content.studySets.learningPaths.length} маршрутов. Начинайте отсюда, если не знаете ID конкретного паттерна.` : `${content.studySets.sets.length} study sets are grouped into ${content.studySets.learningPaths.length} learning paths. Start here when you know what you want to practise but not a pattern ID.`}</p></div><ol>${paths}</ol></div></details>`;
  const marker = '<div data-practice-discovery-slot></div>';
  if (!html.includes(marker)) throw new Error(`Could not locate practice discovery slot in ${relative}`);
  write(relative, html.replace(marker, `${section}${marker}`));
}
