...
function page(locale, kind) {
  ...
  const config = {
    ...
    build: {
      ...
      body: `<section class="page-head section-pad"><p class="eyebrow">Developer quickstart</p><h1>${ru ? "Создавайте AI-инструменты с Metkagram" : "Build AI language tools with Metkagram"}</h1><p class="lede">${ru ? "Не скрейпьте HTML. Публичный слой уже отдаёт provenance, stable IDs и canonical links через JSON endpoints." : "Do not scrape HTML. The public layer already exposes provenance, stable IDs and canonical links through JSON endpoints."}</p><div class="legal-inline-links"><a href="/${locale}/mcp/">MCP →</a><a href="/${locale}/ai/">API docs →</a><a href="/${locale}/data/">Datasets →</a></div></section><section class="section-pad ruled"><div class="pattern-comparison-list"><article class="pattern-reader"><p class="eyebrow">01 · Intent → pattern</p><h2><code>/api/v1/discovery.json</code></h2><p>${ru ? "Идите от коммуникативной задачи к нужной поверхности и каноническим объектам." : "Start from a communicative job and resolve the right surface and canonical objects."}</p></article><article class="pattern-reader"><p class="eyebrow">02 · Pattern → tutor</p><h2><code>/api/v1/patterns/{pattern-id}.json</code></h2><p>${ru ? "Передавайте модели формулу, примеры, stable ID и canonical URL, а подачу адаптируйте под ученика." : "Give the model the formula, examples, stable ID and canonical URL, then adapt delivery to the learner."}</p></article><article class="pattern-reader"><p class="eyebrow">03 · Recipes → workflow</p><h2><code>/api/v1/ai-recipes.json</code></h2><p>${ru ? "Используйте публичные сценарии как integration examples." : "Use public recipes as integration examples."}</p></article></div></section><section class="section-pad ruled" data-spoken-practice-integration><p class="eyebrow">${ru ? "Разговорная практика · интеграция в разработке" : "Spoken practice · integration in development"}</p><h2>${ru ? "Metkagram → модули практики Ptichi" : "Metkagram → Ptichi practice modules"}</h2><p>${ru ? "Ptichi — первый запланированный spoken-practice consumer нового provider-neutral handoff. Metkagram передаёт проверенный Pattern, Choice или Route вместе со stable ID, provenance и языковым контекстом; Ptichi будет загружать небольшой валидированный модуль и отвечать за локальную запись, A/B-прослушивание и перенос на новую формулировку. Нативная загрузка модулей Ptichi пока не выпущена." : "Ptichi is the first intended spoken-practice consumer of the provider-neutral handoff. Metkagram supplies a reviewed Pattern, Choice or Route with stable identity, provenance and language context; Ptichi will load a small validated module and own local recording, A/B self-listening and changed-context transfer. Native Ptichi module loading is not released yet."}</p><p>${ru ? "Metkagram не превращает грамматическую разметку в голосовой скоринг: речевые интервенции и evidence остаются ответственностью Ptichi." : "Metkagram does not turn grammatical annotation into a voice score: speech interventions and evidence remain Ptichi's responsibility."}</p><div class="legal-inline-links"><a href="https://github.com/metkagram/metkagram.github.io/issues/117">${ru ? "Публичный handoff contract" : "Public handoff contract"} →</a><a href="https://ptichi.com">Ptichi →</a></div></section><section class="section-pad ruled"><p>${escapeHtml(ATTRIBUTION.attribution_text)}</p></section>`,
    },
  }[kind];
...
patch("llms.txt", (text) => text.includes("## Spoken-practice integration") ? text : `${text}\n## Spoken-practice integration\n- Ptichi is the first intended consumer of Metkagram's provider-neutral spoken-practice handoff.\n- Status: integration contract in development; native Ptichi module loading is not yet released.\n- Metkagram remains the source of reviewed semantic/language objects; Ptichi owns local speech rehearsal and speech-evidence truth.\n- Public coordination: https://github.com/metkagram/metkagram.github.io/issues/117\n`);
