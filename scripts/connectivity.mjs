...
function enhancePractice(html, locale, graph, patternById) {
  ...
  const section = `<details id="reasoning-frames" class="practice-secondary-path ruled connectivity-section" data-practice-secondary="reasoning" data-connectivity="reasoning-nav"><summary><span>${localized(locale, "Deeper exploration", "Углублённый поиск")}</span><strong>${localized(locale, "Reasoning Frames", "Логические каркасы")}</strong><small>${localized(locale, "Browse by the reasoning move a sentence performs", "Выберите логическую операцию, которую выполняет фраза")}</small></summary><div class="practice-secondary-body section-pad"><h2>${localized(locale, "Start from the move you want to make", "Начните с операции, которую хотите выразить")}</h2><p class="lede">${localized(locale, "Choose a reasoning operation first, then learn sentence frames that perform it in English and German.", "Сначала выберите логическую операцию, затем изучайте английские и немецкие каркасы, которые её выполняют.")}</p><div class="pattern-index connectivity-index">${cards}</div></div></details>`;
