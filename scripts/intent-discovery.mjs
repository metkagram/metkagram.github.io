...
function practiceIntentSection(locale) {
  ...
  const cards = featured.map((intent) => `<a href="/${locale}/learn/${intent.id}/"><span class="document-number">${escapeHtml(intent.move)}</span><span><strong>${escapeHtml(intentTitle(intent, locale))}</strong><small>${escapeHtml(locale === "ru" ? intent.description_ru : intent.description_en)}</small></span><span aria-hidden="true">→</span></a>`).join("");
  return `<section class="section-pad ruled" data-intent-discovery="practice"><p class="eyebrow">${localized(locale, "Intent discovery", "Поиск по намерению")}</p><h2>${localized(locale, "What do you want the sentence to do?", "Что должна сделать ваша фраза?")}</h2><p class="lede">${localized(locale, "Start from a goal such as disagreeing politely or correcting an assumption. Metkagram maps it to a reasoning move and reusable English/German frames.", "Начните с цели: например, вежливо не согласиться или исправить предположение. Metkagram свяжет её с логической операцией и английскими/немецкими каркасами.")}</p><div class="pattern-index">${cards}</div><p><a href="/${locale}/learn/">${localized(locale, "Browse all learner task guides", "Все страницы речевых задач")} →</a> · <a href="/${locale}/practice/intents/">${localized(locale, "Open the full intent map", "Открыть карту намерений")} →</a></p></section>`;
}
...
function enhanceReasoningPattern(locale, pattern) {
  ...
  const cards = intents.map((intent) => `<a href="/${locale}/learn/${intent.id}/"><span class="document-number">${escapeHtml(intent.move)}</span><span><strong>${escapeHtml(intentTitle(intent, locale))}</strong><small>${escapeHtml(locale === "ru" ? intent.description_ru : intent.description_en)}</small></span><span aria-hidden="true">→</span></a>`).join("");
