...
function injectStyles() {
  ...
  style.textContent = `
  .active-practice{margin:clamp(2rem,5vw,4rem) 0;padding:clamp(1.5rem,3vw,2.5rem);border:1px solid var(--line,#d8d8d2);border-radius:10px;background:var(--surface,#fff)}
  ...
  .practice-language-switch{display:flex;gap:.45rem;margin:1rem 0}.practice-language-switch button,.practice-actions button,.practice-rating button{font:inherit;border:1px solid var(--line,#cfcfc8);background:var(--surface,#fff);border-radius:999px;padding:.62rem .9rem;cursor:pointer}.practice-language-switch button[aria-pressed=true]{background:#111;color:#fff;border-color:#111}
  .practice-frame{display:grid;gap:0;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));margin:1.25rem 0;border-top:1px solid var(--line,#d8d8d2);border-left:1px solid var(--line,#d8d8d2)}.practice-frame>div{padding:1rem;border-right:1px solid var(--line,#d8d8d2);border-bottom:1px solid var(--line,#d8d8d2);background:var(--surface,#fff)}.practice-frame small{display:block;margin-bottom:.35rem;text-transform:uppercase;letter-spacing:.08em;color:var(--muted,#666)}.practice-frame code{white-space:normal;font-size:1rem}
  .practice-answer{display:block;margin-top:1.25rem}.practice-answer span{display:block;font-weight:700;margin-bottom:.45rem}.practice-answer textarea{width:100%;min-height:7.5rem;resize:vertical;font:inherit;line-height:1.5;padding:.9rem;border:1px solid var(--line,#cfcfc8);border-radius:8px;background:var(--surface,#fff);color:inherit}
  ...
  .practice-feedback{margin-top:1rem;padding:1rem;border-left:2px solid #111;background:#fafaf8;border-radius:0}.practice-feedback p{margin:.25rem 0}.practice-feedback small{color:var(--muted,#666)}.practice-anchor-list{display:flex;flex-wrap:wrap;gap:.35rem;margin-top:.55rem}.practice-anchor-list code{padding:.25rem .4rem;border-radius:4px;background:rgba(0,0,0,.05)}
...
function patternIdFromPath() {
  return document.querySelector('.pattern-page')?.dataset.patternId?.toUpperCase() || null;
}
...
async function setupPatternPractice() {
  ...
  section.querySelectorAll('[data-practice-rate]').forEach((button) => button.addEventListener('click', () => {
    ...
    state.items[key] = { patternId: id, patternPath: window.location.pathname, language, ...next, attempts };
    writeState(state);
