...
test("wave two structural formulas do not duplicate the previous catalogue",()=>{
...
test("wave two routes are registered and unique in both locales",()=>{
  for(const locale of ["en","ru"]){
    ...
    for(const id of speakingExpansionWave2SetIds) routes.push(studySetPath(locale,canonicalContent.studySets.sets.find(s=>s.id===id)));
    assert.equal(new Set(routes).size,66);
  }
});
test("wave two missing raw annotations are synthesized only from the explicit pending ledger",()=>{
  ...
  for(const p of patterns){
    ...
    for(const l of p.langs) for(const key of ["primary","1","2","3","4","5"]){
      ...
      assert.ok(r,full);assert.equal(r.validation.status,"pending");assert.equal(r.validation.reason,"awaiting_local_annotation");assert.equal(r.validation.generator,"none");assert.deepEqual(r.spans,[]);
