...
const PATTERN_ROUTE = patternPath("en", "CLF041");
const OVERLAP_ROUTE = "/en/practice/patterns/example-pattern-xpr000/";
const SET_ROUTE = "/en/practice/sets/qualification-and-hedging/";
...
const basePayload = {
  ...
  rows: [
    ...
    { url: `https://metkagram.github.io${SET_ROUTE}`, clicks: 1, impressions: 180, ctr: 0.0056, position: 14.2, crawled: true, indexed: true, age_days: 210 },
    { url: `https://metkagram.github.io${OVERLAP_ROUTE}`, clicks: 0, impressions: 4, ctr: 0, position: 42.1, crawled: true, indexed: true, age_days: 240, consolidation_group: "reviewed-overlap" },
    ...
    { url: `https://metkagram.github.io${PATTERN_ROUTE}`, clicks: 0, impressions: 8, ctr: 0, position: 55, crawled: true, indexed: true, age_days: 365 }
  ]
};
...
test("route classification distinguishes Atlas, sets, patterns and editorial surfaces", () => {
  ...
  assert.equal(classifyRoute("/en/practice/sets/hypothesis-testing/"), "study_set");
  assert.equal(classifyRoute(PATTERN_ROUTE), "pattern");
...
test("aggregate report produces a bounded editorial decision queue", () => {
  ...
  assert.equal(byRoute.get(PATTERN_ROUTE).action, "observe");
  assert.equal(byRoute.get(SET_ROUTE).action, "improve");
  assert.equal(byRoute.get("/en/patterns/thinking-framing-uncertainty-and-decisions/").action, "expand");
  assert.equal(byRoute.get(OVERLAP_ROUTE).action, "consolidate");
...
test("core learning pages are never noindex candidates without explicit permission", () => {
  const result = recommendSearchAction({
    url: `https://metkagram.github.io${PATTERN_ROUTE}`,
    clicks: 0,
    impressions: 0,
    indexed: true,
    crawled: true,
