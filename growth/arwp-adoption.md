...
Baseline: 2026-09-06  
Site: https://metkagram.github.io/  
...
- Public Search crawling: allowed for editorially indexable public routes; low-confidence or insufficient pattern variants can remain `noindex,follow` under the repository's explicit indexability policy.
...
| --- | --- | --- | --- | --- |
| `search-foundation-first` | Static HTML, canonicals, hreflang, sitemap/robots generation, explicit indexability policy and deployment verification already exist. | Keep eligibility checks ahead of optional agent metadata; retain truthful noindex decisions for weak pattern variants. | `npm run verify`, deployment smoke, recurring ARWP Growth artifact. | Indexed priority routes, Search impressions; owner data only. |
...
5. Keep current crawler/reuse rights unchanged; this rollout does not silently broaden training or bulk-reuse permission.
...
- Recurring Growth audit: `.github/workflows/arwp-growth.yml`.
