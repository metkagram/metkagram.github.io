...
export function patternPage(locale, pattern, serviceAnnotations = {}) {
  ...
  const primaryCard = `<section class="pattern-reference-card"><header class="learning-section-label"><span aria-hidden="true">01</span><p class="eyebrow">${t.patternStepFormula}</p></header><div class="pattern-formulas"><div><span class="language-code">EN · ${t.english}</span><code>${escapeHtml(english?.formula || "")}</code></div><div><span class="language-code">DE · ${t.german}</span><code>${escapeHtml(german?.formula || "")}</code></div></div></section>`;
