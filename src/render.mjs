...
import { getLanguage, interfaceLocales } from "./language-registry.mjs";
import { ATTRIBUTION, getDatasetVersion, getReleaseDate } from "./provenance.mjs";
...
import { frameFamilyFor, practiceCountLabel } from "./frame-families.mjs";
    ...
    "@graph": [
      {
        ...
        contactPoint: {
          ...
          availableLanguage: interfaceLocales.map((code) => getLanguage(code).nativeName)
        },
        sameAs: [ATTRIBUTION.source_repository]
      },
      {
        "@type": "WebSite",
        "@id": `${SITE_URL}/#website`,
...
export function patternPage(locale, pattern, serviceAnnotations = {}) {
  ...
  const family = frameFamilyFor(pattern);
  const familyContext = family ? `<aside class="pattern-family-context"><p class="eyebrow">${locale === "ru" ? "Вариант общего каркаса" : "A context for a shared frame"}</p><p>${escapeHtml(locale === "ru" ? family.title_ru : family.title_en)}. <a href="${studySetPath(locale, { id: family.set_id })}#frame-families">${locale === "ru" ? "Каркас и все варианты" : "View the frame and all variants"} →</a></p></aside>` : "";
  ...
  const body = `<article class="pattern-page section-pad" data-pattern-id="${escapeHtml(pattern.id)}">${breadcrumbs(locale, [{ href: `/${locale}/`, label: t.home }, { href: `/${locale}/practice/`, label: t.navPractice }, { href: pathname, label: title }])}<header class="pattern-page-head"><p class="eyebrow">B2–C1 · ${escapeHtml(pattern.group_id)} · ${escapeHtml(pattern.id)}</p><h1>${escapeHtml(title)}</h1>${russianDescription}${familyContext}</header><div class="pattern-comparison">${primaryCard}${variations}</div></article>`;
  ...
  return layout({ locale, pathname, title: metaTitle, description: metaDescription, body, type: "article", bodyClass: "pattern-reader-body", structuredData: [breadcrumbJson(pathname, title, locale), { "@context": "https://schema.org", "@type": "LearningResource", name: title, identifier: pattern.id, educationalLevel: "B2–C1", teaches: pattern.formulas || pattern.langs.map((lang) => lang.formula), inLanguage: pattern.langs.map((lang) => lang.lang), url: `${SITE_URL}${pathname}`, isPartOf: { "@type": "LearningResource", "@id": `${SITE_URL}${studySetPath(locale, { id: pattern.set_id })}#learning-resource`, url: `${SITE_URL}${studySetPath(locale, { id: pattern.set_id })}` } }] });
}
...
export function studySetPage(locale, set, patterns) {
  ...
  const countLabel = practiceCountLabel(locale, set, patterns);
  const body = `${breadcrumbs(locale, [{ href: `/${locale}/`, label: t.home }, { href: `/${locale}/practice/`, label: t.navPractice }, { href: pathname, label: title }])}<section class="page-head section-pad compact study-set-head"><p class="eyebrow">B2–C1 · ${escapeHtml(set.id)} · ${escapeHtml(countLabel)}</p><h1>${escapeHtml(title)}</h1><p class="lede">${escapeHtml(description)}</p></section><section class="pattern-index section-pad ruled">${patterns.map((pattern, index) => `<a href="${patternPath(locale, pattern)}"><span class="document-number">${String(index + 1).padStart(2, "0")}</span><span><strong>${escapeHtml(patternTitle(pattern, locale))}</strong><small>${escapeHtml(pattern.id)} · ${escapeHtml(pattern.langs.map((lang) => lang.formula).join(" / "))}</small></span><span aria-hidden="true">↗</span></a>`).join("")}</section>${shareBar(locale, pathname, title)}`;
  return layout({ locale, pathname, title: `${title} — ${countLabel} | Metkagram`, description, body, structuredData: [breadcrumbJson(pathname, title, locale), { "@context": "https://schema.org", "@type": "LearningResource", name: title, identifier: set.id, description, url: `${SITE_URL}${pathname}`, learningResourceType: locale === "ru" ? "Тематический сет языковых паттернов" : "Language pattern topic set", educationalLevel: "B2–C1", inLanguage: corpusLanguages(), teaches: patterns.slice(0, 12).map((pattern) => pattern.langs.find((lang) => lang.lang === "en")?.formula || pattern.id), isAccessibleForFree: true, numberOfItems: patterns.length }] });
}
...
export function aboutPage(locale) {
  ...
  const body = `<section class="page-head section-pad"><p class="eyebrow">M: · project notes</p><h1>${t.aboutTitle}</h1><p class="lede">${t.aboutIntro}</p></section><section class="about-sections section-pad ruled"><article id="license"><h2>${t.license}</h2><p>${locale === "ru" ? "Metkagram открыт для просмотра и обычного учебного использования сайта, но код, новые версии корпуса, схема разметки и исследовательские материалы по умолчанию не являются open source или open data. Для существенного повторного использования, исследований с копией корпуса и коммерческой интеграции требуется отдельное разрешение." : "Metkagram is publicly inspectable and the hosted learning site remains available to end users, but the source code, new corpus revisions, annotation scheme and research materials are not open source or open data by default. Substantial reuse, corpus-based research and commercial integration require separate permission."}</p><a href="/${locale}/licensing/">${locale === "ru" ? "Права и лицензирование" : "Licensing & rights"} →</a></article><article id="privacy"><h2>${t.privacy}</h2><p>${t.privacyText}</p></article><article><h2>${t.source}</h2><p>${t.connected}</p><a href="${ATTRIBUTION.source_repository}">GitHub ↗</a></article></section>`;
  return layout({ locale, pathname, title: locale === "en" ? "About Metkagram grammar markup" : "О разметке грамматики Metkagram", description: t.aboutIntro, body, structuredData: [breadcrumbJson(pathname, t.aboutTitle, locale), { "@context": "https://schema.org", "@type": "Organization", "@id": `${SITE_URL}/#organization`, name: "Metkagram", url: SITE_URL, sameAs: [ATTRIBUTION.source_repository] }] });
}
...
export function ideasPage(locale) {
  ...
  const c = ru ? {
    ...
    ideas: [
      ...
      ["Язык или контент", "Новая коллекция, язык, набор примеров или редакторская проверка, повышающая качество публично доступного для изучения корпуса."],
    ...
    boundaries: "Публичный учебный каталог остаётся доступным на сайте; существенное переиспользование регулируется текущими условиями лицензирования. Партнёры не управляют выводами исследований и редакционными решениями; вклад, лицензия и происхождение данных обозначаются прозрачно.",
    ...
    ideas: [
      ...
      ["Language or content", "A new collection, language, example set, or editorial review that improves the quality of the publicly inspectable corpus."],
    ...
    boundaries: "The public learner-facing catalogue remains accessible on the hosted site; substantial reuse follows the current licensing terms. Partners do not control research findings or editorial decisions; contributions, licence terms, and data provenance are made explicit.",
...
export function supportPage(locale, counts) {
  ...
  const c = ru ? {
    eyebrow: "Metkagram · партнёрство и финансирование", title: "Помогите превратить исследовательский языковой корпус в проверяемую учебную систему.", intro: "Metkagram объединяет модели B2–C1, выбранные примеры с разметкой, Pattern Lens, Pattern Atlas, проверяемые сравнения и статический слой для агентов. Нужны конкретные исследовательские, преподавательские, EdTech и лицензионные пилоты, без выдуманных заявлений об аудитории или результатах.", snapshotTitle: "Проект сегодня", thesisTitle: "Почему проект может масштабироваться", thesis: [["Структурированный актив", "Ценность хранится не в отдельных страницах, а в версионированном корпусе, схеме разметки и связях между предложениями, паттернами и наборами."], ["Два канала продукта", "Один проверенный pattern ID может использоваться в Lens, Atlas, сравнении, практике, преподавательском экспорте и ответе ИИ без конкурирующих определений."], ["Статическая экономика", "Основной сайт работает без runtime-бэкенда: публикация воспроизводима, обслуживание недорого, а контент остаётся доступным офлайн и на GitHub Pages."], ["Локализация по контракту", "Новые языки добавляются через единые поля, валидаторы и fallback на английский, а не через копирование интерфейса вручную."]], prioritiesTitle: "На что пойдёт следующий ресурс", priorities: [["01", "Проверка метода", "Пилотное исследование с заранее зарегистрированными гипотезами, контрольными условиями и открытым отчётом."], ["02", "Редакционное качество", "Лингвистическая проверка корпуса, согласие разметчиков и исправление слабых или искусственных примеров."], ["03", "Новые языки", "Локализационный pipeline, языковые правила и первый дополнительный целевой язык после English и German."], ["04", "Удержание учащихся", "Личные маршруты, активное извлечение и измеримые циклы возвращения к паттернам."]], modelsTitle: "Форматы участия", ways: [["Преподавательский / EdTech пилот", "Проверить Metkagram в реальном учебном или AI-tutor сценарии с конкретным результатом и обратной связью."], ["Исследовательское партнёрство", "Провести учебный или NLP-эксперимент на корпусе с воспроизводимой методикой и явной атрибуцией."], ["Контент и экспертиза", "Помочь с редактурой, лингвистической проверкой, сценариями использования или новыми языковыми коллекциями."], ["Технологическое партнёрство", "Поддержать API, агентские интеграции, инструменты качества данных, доступность или хостинг."]], promiseTitle: "Что остаётся защищённым", promise: ["Публичный учебный каталог остаётся доступным на сайте; существенное переиспользование регулируется текущими условиями лицензирования.", "Спонсоры не получают права менять результаты исследований, разметку или рекомендации под свои интересы.", "Партнёрства и вклад обозначаются прозрачно; данные сохраняют происхождение и атрибуцию.", "Показатели аудитории, эффекта и выручки публикуются только тогда, когда их можно подтвердить."], contactTitle: "Начнём с конкретного предложения", contact: "Откройте контакты Metkagram и выберите email или LinkedIn. Укажите, кто вы, какой ресурс или компетенцию предлагаете и какой результат хотите получить — в ответ мы предложим подходящий этап, артефакт и показатель успеха.", contactLink: "Обсудить инвестиции или партнёрство", sourceLink: "Проверить публичный репозиторий"
  } : {
    eyebrow: "Metkagram · partnerships and funding", title: "Turn a research language corpus into a testable learning system.", intro: "Metkagram combines reusable B2–C1 patterns, selected annotated examples, Pattern Lens, Pattern Atlas, reviewed contrasts and a static agent-facing reference layer. We are looking for concrete research, teaching, EdTech and licensing pilots, without inventing traction claims.", snapshotTitle: "Project snapshot", thesisTitle: "Why this can compound", thesis: [["Structured asset", "Value lives beyond individual pages in a versioned corpus, annotation schema and links between sentences, patterns and study sets."], ["Two product surfaces", "One reviewed pattern ID can travel through Lens, Atlas, contrasts, practice, teacher exports and AI-tutor answers without competing definitions."], ["Static economics", "The core site needs no runtime backend: publication is reproducible, maintenance stays lean, and content remains available on GitHub Pages."], ["Localization by contract", "New languages can enter through shared fields, validators and English fallback rules instead of copied interfaces."]], prioritiesTitle: "What the next resources unlock", priorities: [["01", "Method validation", "A preregistered pilot with comparison conditions, defined outcomes and a public report."], ["02", "Editorial quality", "Linguistic review, inter-annotator agreement and correction of weak or artificial examples."], ["03", "New languages", "A localization pipeline, language-specific rules and the first target language beyond English and German."], ["04", "Learner retention", "Personal paths, active retrieval and measurable return loops around useful patterns."]], modelsTitle: "Ways to participate", ways: [["Teacher / EdTech pilot", "Test Metkagram in a real teaching or AI-tutor workflow with a concrete outcome and editorial feedback."], ["Research partnership", "Run a learning or NLP experiment on the corpus with a reproducible approach and clear attribution."], ["Content and expertise", "Contribute editorial work, linguistic review, use cases or new language collections."], ["Technology partnership", "Support API access, agent integrations, data-quality tooling, accessibility or hosting."]], promiseTitle: "What stays protected", promise: ["The public learner-facing catalogue remains accessible on the hosted site; substantial reuse follows the current licensing terms.", "Sponsors do not receive the right to alter research findings, annotations or recommendations for their own interests.", "Partnerships and contributions are acknowledged transparently; data keeps its provenance and attribution.", "Audience, efficacy and revenue metrics are published only when they can be verified."], contactTitle: "Start with a concrete proposal", contact: "Open Metkagram contact options and choose email or LinkedIn. Tell us who you are, the resource or expertise you can contribute and the outcome you want; we will reply with a suitable milestone, artifact and measure of success.", contactLink: "Discuss investment or partnership", sourceLink: "Inspect the public repository"
  };
  const metrics = projectMetrics(counts, locale);
...
export function aiPage(locale, content, counts, apiRoutes) {
  ...
  const citeWeb = citations.web;
  const citeAcademic = citations.academic;
  const citeAi = citations.ai_answer;
...
const id = "C1OP001";
const response = await fetch(\`${SITE_URL}/api/v1/patterns/\${id.toLowerCase()}.json\`);
...
<section class="ai-section section-pad ruled" id="collaborate"><div><p class="eyebrow">08 · ${t.aiCollaborate}</p><h2>${t.aiCollaborate}</h2></div><p class="lede">${en ? "We welcome research, teaching, content and language-data collaborations that keep attribution intact." : "Мы приветствуем исследовательские, образовательные, контентные и языковые проекты с сохранением атрибуции."}</p><nav class="download-list"><a href="${ATTRIBUTION.contact_url}" target="_blank" rel="noreferrer">${en ? "Contact MetalHatsCats" : "Связаться с MetalHatsCats"}</a><a href="${ATTRIBUTION.source_repository}">${en ? "GitHub repository" : "Репозиторий GitHub"}</a></nav></section>`;
  ...
  const structuredData = [
    ...
    { "@context": "https://schema.org", "@type": "LearningResource", name: title, educationalLevel: "B2–C1", inLanguage: [...new Set([...corpusLanguages(), ...interfaceLocales])], url: `${SITE_URL}${pathname}`, isPartOf: { "@id": `${SITE_URL}/#website` } },
...
<head>
  ...
  <meta name="metkagram-rights" content="${ATTRIBUTION.rights_status}">
  <link rel="license" href="/en/licensing/">
