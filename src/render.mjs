...
export function patternPage(locale, pattern, serviceAnnotations = {}) {
  ...
  const primaryCard = `<section class="pattern-reference-card"><header class="learning-section-label"><span aria-hidden="true">01</span><p class="eyebrow">${t.patternStepFormula}</p></header><div class="pattern-formulas"><div><span class="language-code">EN · ${t.english}</span><code>${escapeHtml(english?.formula || "")}</code></div><div><span class="language-code">DE · ${t.german}</span><code>${escapeHtml(german?.formula || "")}</code></div></div><article class="pattern-comparison-card pattern-primary-example"><div class="pattern-comparison-sentences">${renderSentence(english, english?.card, "EN · " + t.english)}${renderSentence(german, german?.card, "DE · " + t.german)}</div>${renderTranslation(english?.translation || german?.translation)}</article></section>`;
