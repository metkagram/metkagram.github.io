...
export function localeHome(locale, content) {
  ...
  const body = `<section class="home-hero section-pad">
    <div class="home-intro"><div class="home-copy"><p class="eyebrow">${t.homeEyebrow}</p><h1>${t.statement}</h1><p class="home-kicker">${t.homeKicker}</p><p class="lede">${t.homeIntro}</p><div class="home-cta"><a class="primary-link" href="/${locale}/explore/">${t.homeExplore} <span aria-hidden="true">→</span></a><a class="text-link" href="/${locale}/method/">${t.homeMethodLink} <span aria-hidden="true">→</span></a></div></div><figure class="home-specimen" aria-label="${t.homeKicker}"><figcaption>01 · ${t.homeJourneyUnderstand}</figcaption>${annotatedPreview()}</figure></div>
  </section>
