...
export function patternPage(locale, pattern, serviceAnnotations = {}) {
  ...
  const body = `<article class="pattern-page section-pad" data-pattern-id="${escapeHtml(pattern.id)}">${breadcrumbs(locale, [{ href: `/${locale}/`, label: t.home }, { href: `/${locale}/practice/`, label: t.navPractice }, { href: pathname, label: title }])}<header class="pattern-page-head"><p class="eyebrow">B2–C1 · ${escapeHtml(pattern.group_id)} · ${escapeHtml(pattern.id)}</p><h1>${escapeHtml(title)}</h1>${localizedDescription}</header><div class="pattern-comparison">${primaryCard}${variations}</div></article>`;
  const metaTitle = identifiedMetaTitle(title, pattern.id);
  const metaDescription = description || (locale === "en"
