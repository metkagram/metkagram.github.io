...
function footer(locale) {
  ...
  return `<footer class="site-footer">
    ...
    <nav aria-label="Footer"><a href="/${locale}/apps/">${t.apps}</a><a href="/${locale}/ai/">${t.forAiDevelopers}</a><a href="/${locale}/support/">${t.support}</a><a href="/${locale}/legal/privacy/">${t.privacy}</a><a href="/${locale}/legal/terms/">${t.terms}</a><a href="https://github.com/metkagram/metkagram.github.io">${t.source}</a><a href="/${locale}/history/">${t.history}</a><a href="/${locale}/roadmap/">${t.roadmap}</a><a href="/${locale}/roadmap/#changelog">${t.changelog}</a><a href="/${locale}/about/#license">${t.license}</a></nav>
    <p>${t.connected}</p>
  </footer>`;
}
...
export function patternPage(locale, pattern, serviceAnnotations = {}) {
  ...
  const languageCards = pattern.langs.map((lang) => {
    ...
    const primary = renderCanonicalText(card, (span, text) => `<mark class="pattern-token">${escapeHtml(text)}<small>${escapeHtml(span.label)} · ${escapeHtml(span.role || span.type)}</small></mark>`);
    return `<section class="pattern-language-card" id="${lang.lang}" data-target-language="${lang.lang}"><header><p class="language-code">${lang.lang.toUpperCase()}</p><h2>${lang.lang === "en" ? t.english : t.german}</h2></header><dl class="pattern-reference"><div><dt>${t.formula}</dt><dd><code>${escapeHtml(lang.formula)}</code></dd></div><div class="pattern-primary-example"><dt>${t.example}</dt><dd class="canonical-example">${primary}</dd>${lang.translation ? `<p lang="ru">${escapeHtml(lang.translation)}</p>` : ""}</div></dl>${card.examples?.length ? `<section class="pattern-variations" aria-label="${t.examples}"><h3>${t.examples}<small>${card.examples.length}</small></h3><ol class="example-list">${card.examples.map((example) => `<li><p>${renderCanonicalText(example, (span, text) => `<mark class="pattern-token">${escapeHtml(text)}<small>${escapeHtml(span.label)}</small></mark>`)}</p>${example.translation ? `<small lang="ru">${escapeHtml(example.translation)}</small>` : ""}</li>`).join("")}</ol></section>` : ""}</section>`;
  }).join("");
