...
const C1_RU_TITLES = {
  AGR: "Я вижу смысл в [X], хотя я бы [Y]",
  ARG: "Аргумент в пользу [X] основан на предположении, что [Y]",
  CAU: "Поскольку [X], [Y], из-за чего [Z]",
  CLR: "Если сформулировать [X] точнее, [Y]",
  CMP: "По сравнению с [X], [Y] даёт [Z]",
  CND: "Если бы [X] произошло раньше, [Y], возможно, [Z]",
  DGR: "Не только [X], но и [Y] — усилительная инверсия",
  EVD: "Имеющиеся данные указывают на [X], хотя [Y]",
  HED: "Было бы преждевременно заключать, что [X], прежде чем [Y]",
  LNK: "Тем не менее [X]; в более широком контексте [Y]",
  MOD: "Чтобы [X] сработал, [Y] должен [Z]",
  NEG: "Можем ли мы взять [X] за основу компромисса при условии, что [Y]?",
  OPI: "После тщательного рассмотрения я считаю, что [X], особенно учитывая [Y]",
  PRB: "Есть реальная вероятность, что [X], если [Y]",
  PRO: "Пишу, чтобы уточнить [X], прежде чем [Y]",
  PSV: "Перед [X] с объектом [Y] выполняется действие",
  QNN: "Что могло бы помешать [X] выполнить [Y]?",
  REP: "[Источник] подчеркнул, что [X], а не [Y]",
  STO: "Оглядываясь назад, [X] стал моментом, когда [Y]",
  TAS: "Действие продолжается с момента [Y] и остаётся актуальным сейчас"
};
